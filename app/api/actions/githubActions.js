...
async function getUserRepos() {
    try {
      const response = await githubApi.get('/user/repos', { params: { per_page: 100 } }); // Adjust per_page as needed
  ...
  export const getGithubCommitsForToday = async (userId) => {
    ...
    const allCommitsToday = commitResults.filter(result => result.commits.length > 0);

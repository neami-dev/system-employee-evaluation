...
useEffect(() => {
  ...
  if (repoSelected !== undefined) {
    auth.onAuthStateChanged((user) => {
      ...
      setRepofirebaseGithub(repoSelected, user?.uid).then((res) => {
        console.log(res);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-indigo-100 flex flex-col justify-center items-center p-4">
      ...
      <ul className="text-lg text-gray-800 list-none space-y-2 mb-8">
        ...
        <li className="flex items-center my-5">
          <CheckCircle2 className={`${firebaseRepo == "OK" ? "text-green-500": "text-gray-500"} min-h-9 min-w-9  mr-2`} /> {/* "DONE" icon */}
          <span className={`${firebaseRepo == "OK" ? "text-green-600": "text-gray-600"}  text-[25px] font-semibold`}>Choose the repo you are working on :</span>
        </li>
        <li className="mt-3 ml-[40px]">
          <Dialog >
                        <DialogTrigger asChild>
                            ...
                            {/* Choose repositorie */}
                                {repoSelected ? repoSelected:"Choose repositorie"}
                            </Button>
                        </DialogTrigger>
        ...
        <li className="flex items-center mb-9">
          <Button className={`${firebaseRepo == "OK" ? "bg-black hover:bg-black": "bg-gray-400 hover:bg-gray-400"} text-[20px]  font-semibold mt-9`} type="button" onClick={() => firebaseRepo == "OK" ? router.push("/services"):null}>
            submit
          </Button>
        </li>
      </ul>
